...
# 1. Manifest란 무엇인가
  ...
  ## 나머지 항목들
  >
   > 1.<action> 인텐트 필터에 작업을 추가합니다.
   
   > 2.<activity> 애플리케이션의 시각적 사용자 인터페이스 요소를 구현하는 액티비티를 선언합니다. 

   > 3.<activity-alias> activity의 별칭으로, targetActivity 속성에서 이름이 지정됩니다.
   
   > 4. <application> 애플리케이션 선언입니다. 이 요소는 애플리케이션의 각 구성요소를 선언하고 모든 구성요소에 영향을 줄 수 있는 속성을 가진 하위 요소를 포함합니다.
   
   > 5. <category> 인텐트 필터에 카테고리 이름을 추가합니다.
   
   > 6. <compatible-screens>  애플리케이션이 호환되는 각 화면 구성을 지정합니다.
